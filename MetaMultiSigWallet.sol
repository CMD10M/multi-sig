...
contract MetaMultiSigWallet {
    ...
    //Events when a deposit, transaction or owner is added.
    ...
    //maaping that checks if the msg.sender is the owner in true/false fashion
    mapping(address => bool) public isOwner;
    //After transaction is submitted to contract, other owners will need to approve.
    //# of multi-sig approvals is this uint
    uint public signaturesRequired;
    uint public nonce;
    uint public chainId;

    //Construct that takes the owners address and signatures required.
    constructor(
        uint256 _chainId,
        address[] memory _owners,
        uint _signaturesRequired
    ) {
        //Requires atleast 1 signature
        require(
            _signaturesRequired > 0,
            "constructor: must be non-zero sigs required"
        );
        //For loop that takes the owners address and defines it as the variable i
        signaturesRequired = _signaturesRequired;
        for (uint i = 0; i < _owners.length; i++) {
            address owner = _owners[i];
            //requires atleast 1 owner and owner is unique
    ...
    //restricts function to only be called by the owner
    modifier onlySelf() {
        //requires msg.sender to be the address owner
    ...
    function executeTransaction(
        ...
        uint256 value,
        bytes memory data,
        //the transaction is signed by one of the signers associated with the multisig
        //and added to an array of signatures bytes[] --> this is the array that is "off-chain" to reduce gas
        bytes[] memory signatures
    ) public returns (bytes memory) {
        //this checks if the signature is valid based on the allowed signatories
        require(
            isOwner[msg.sender],
        ...
        //makes sure signatures are unique and pulls from off-chain array
        ...
        //if the signatures are valid then...
        require(
            validSignatures >= signaturesRequired,
            "executeTransaction: not enough valid signatures"
        );

        //... then the transaction is passed to this call function
        //and deployed as a successful transaction\
        //this part is "on-chain"
    ...
    //allows contract to receive ethereum
